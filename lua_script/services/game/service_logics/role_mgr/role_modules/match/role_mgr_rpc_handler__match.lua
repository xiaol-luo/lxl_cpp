...
function RoleMgr:_setup_rpc_handler__match()
    ...
    self.rpc_mgr:set_req_msg_process_fn(GameRpcFn.notify_terminate_match, Functional.make_closure(self._on_rpc_notify_confirm_join_match, self))
...
function RoleMgr:_on_rpc_notify_confirm_join_match(rpc_rsp, role_id, session_id, match_room_id)
    ...
    repeat
        ...
        if not role.match_session_id or role.session_id ~= session_id then
            is_accept = false
            break
        end
...
function RoleMgr:_on_rpc_notify_match_succ(rpc_rsp, role_id, session_id, room_service_key, room_id)
    rpc_rsp:respone()
    local role = self:get_in_game_role(role_id)
    if role then
        if role.match.session_id and role.match.session_id == session_id then
            ...
            role.match:sync_match_state()
            log_debug("RoleMgr:_on_rpc_notify_match_succ %s %s", room_service_key, room_id)
        end
    end
end

...
function LogicMain.start()
    ...
    local cnn = native.LuaPidBinCnn:new()
    local ret = cnn:init(tcp_cnn_logic)
    print("xxx", ret)
    print(cnn)

    g_listen_handler = NetListen:new()
    ...
    Net.listen("0.0.0.0", 1234, g_listen_handler)

    g_cnn_handler_connect = PidBinCnn:new()
    ...
    Net.connect("127.0.0.1", 1234, g_cnn_handler_connect)

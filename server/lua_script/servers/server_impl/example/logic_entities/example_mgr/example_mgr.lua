...
---@class ExampleMgr:GameLogicEntity
ExampleMgr = ExampleMgr or class("ExampleMgr", GameLogicEntity)

function ExampleMgr:_on_init()
    ExampleMgr.super._on_init(self)
    ---@type MatchServiceMgr
    self.server = self.server
end

function ExampleMgr:_on_start()
    ExampleMgr.super._on_start(self)
...
function ExampleMgr:_on_stop()
    ExampleMgr.super._on_stop(self)
    self._rpc_svc_proxy:clear_remote_call()
end

function ExampleMgr:_on_release()
    ExampleMgr.super._on_release(self)
end

function ExampleMgr:_on_update()
    -- log_print("ExampleMgr:_on_update")
end

---@param rpc_rsp RpcRsp
function ExampleMgr:_handle_remote_call_create_role(rpc_rsp, user_id)
    if not user_id then
...
---@param rpc_rsp RpcRsp
function ExampleMgr:_handle_remote_call_query_roles(rpc_rsp, user_id, role_id)
    -- log_print("ExampleMgr:_handle_remote_call_query_roles",  user_id, role_id)

...
function RoleFightHandleClientMsgFns.req_join_match(self, from_gate, gate_netid, role_id, pid, msg)
    ...
    if not role then
        ...
        server_ins.logics.forward_msg:send_msg_to_client(from_gate, gate_netid, Fight_Pid.rsp_join_match, { error_num = 1 })
        return
    end
...
function RoleFightHandleClientMsgFns.req_quit_match(self, from_gate, gate_netid, role_id, pid, msg)
    local role = self:get_role(role_id)
    if not role then
        return server_ins.logics.forward_msg:send_msg_to_client(from_gate, gate_netid, Fight_Pid.req_quit_match, { error_num = 1 })
    end

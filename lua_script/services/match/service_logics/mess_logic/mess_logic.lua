...
function MessLogic:init()
    ...
    local rpc_process_fns_map = {
        [MatchRpcFn.query_service_state] = self._on_query_service_state,
    }
...
function MessLogic:_on_query_service_state(rpc_rsp)
    -- log_debug("MessLogic:_on_query_service_state")
    rpc_rsp:respone()
end

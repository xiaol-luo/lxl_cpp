...
function MatchMgr:_handle_remote_call_create_role(rpc_rsp, user_id)
    if not user_id then
...
---@param rpc_rsp RpcRsp
function MatchMgr:_handle_remote_call_query_roles(rpc_rsp, user_id, role_id)
    -- log_print("MatchMgr:_handle_remote_call_query_roles",  user_id, role_id)

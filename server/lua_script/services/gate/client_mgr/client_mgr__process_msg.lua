...
function ClientMgr:process_req_user_login(netid, pid, msg)
    ...
    repeat
        ...
        client.state = Gate_Client_State.authing
    until true
    ...
    local over_cb = function(co)
        ...
        repeat
            do
                ...
                client.state = Gate_Client_State.manage_role
        ...
        if Error_None ~= error_num then
            local client = self:get_client(netid)
            if client then
                if Gate_Client_State.authing == client.state then
...
function ClientMgr:process_reconnect(netid, pid, msg)
    ...
    local main_logic = function(co, msg)
        ...
        client.state = Gate_Client_State.launch_role
        ...
        client.state = Gate_Client_State.in_game
    ...
    local over_cb = function(co)
        ...
        if Error_None ~= error_num then
            ...
            if client then
                ...
                if Gate_Client_State.launch_role == client.state then
                    client.state = Gate_Client_State.manage_role
                end
            end
        end
...
function ClientMgr:process_req_launch_role(netid, pid, msg)
    ...
    do
        local client = self:get_client(netid)
        if not client or Gate_Client_State.manage_role ~= client.state or not client.user_id then
...
function ClientMgr:_rpc_rsp_req_luanch_role(netid, role_id, rpc_error_num, launch_error_num, game_key, world_role_session_id, ...)
    ...
    do
        ...
        local client = self:get_client(netid)
        if not client or Gate_Client_State.launch_role ~= client.state then
            error_num = Error.Launch_Role.state_not_fit
            break
        end
        client.state = Gate_Client_State.in_game
...
function ClientMgr:process_logout_role(netid, pid, msg)
    ...
    repeat
        ...
        if Gate_Client_State.in_game ~= client.state or not client.launch_role_id or msg.role_id ~= client.launch_role_id then
...
function ClientMgr:_rpc_rsp_logout_role(netid, rpc_error_num, logout_error_num)
    ...
    repeat
        ...
        if Gate_Client_State.in_game ~= client.state then

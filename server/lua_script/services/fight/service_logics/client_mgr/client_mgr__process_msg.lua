...
function ClientMgr:setup_proto_handler()
    ...
    self.client_cnn_mgr:set_process_fn(ProtoId.req_bind_fight, Functional.make_closure(self._on_msg_req_bind_find, self))
...
function ClientMgr:_default_hanle_msg_fn(netid, pid, msg)
    if pid > ProtoId.fight_logic_min_pid and pid < ProtoId.fight_logic_max_pid then
        ...
        if client then
            if Client_State.binded == client and client.fight then
                client.fight:on_client_msg(client, pid, msg)
            else
                log_error("ClientMgr:_default_hanle_msg_fn netid=%s, pid=%s client not binded", pid, netid)
            end
        else
            log_error("ClientMgr:_default_hanle_msg_fn netid=%s, pid=%s, unexpected error for not find client", pid, netid)
        end
        return
    end
...
function ClientMgr:_on_msg_req_bind_find(netid, pid, msg)
    ...
    repeat
        ...
        local fight = self.logic_mgr.fight_mgr:get_fight(msg.fight_id)
        if not fight then

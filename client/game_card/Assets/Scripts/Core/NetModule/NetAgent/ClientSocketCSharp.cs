...
namespace Utopia
{
    public class ClientSocketCSharp : IClientSocket
    {
        public class ThreadParam
        {
            public bool isExit = true; // main write
            public ClientSocketState state = ClientSocketState.Free; // main write
        ...
        protected ClientSocketState state { get { return m_threadParam.state; } }
        protected int errno { get { return m_threadParam.errno; } }
        protected string errmsg { get { return m_threadParam.errmsg; } }
        private System.Action m_asyncDoCnnCb;
        private System.Action<List<byte[]>> m_recvDataCb;
        private System.Action m_closeCb;

        public ClientSocketCSharp() : this(string.Empty, 0)
        {
        }
        public ClientSocketCSharp(string _host, int _port)
        {
        ...
        {
            if (ClientSocketState.Free != m_threadParam.state)
                return false;
            Socket socket = null;
            try
            {
                m_threadParam.state = ClientSocketState.Connecting;
        ...
        {
            if (ClientSocketState.Connected == state && offset >= 0 && data_len > 0 && data.Length >= offset + data_len)
            {
        ...
        public void UpdateIO()
        {
            ...
            if (ClientSocketState.Connected != m_threadParam.state)
                return;
            if (0 != m_threadParam.errno)
            {
                this.Close();
                this.m_threadParam.state = ClientSocketState.Error;
            }
            else
            {
        ...
        public ClientSocketState GetState()
        {
            return state;
        }

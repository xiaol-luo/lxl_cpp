return {
    "libs.log",
    "libs.class",
    "libs.functional",
    "libs.sequencer",
    "libs.string_ext",
    "libs.assert",
    "libs.base64",
    "libs.path_ext",
    "libs.math_ext",
    "libs.random",
    "libs.table_ext",
    "libs.type_check",
    "libs.hotfix",

    "libs.coroutine_ex.coroutine_ex_def",
    "libs.coroutine_ex.coroutine_ex",
    "libs.coroutine_ex.coroutine_ex_mgr",

    "libs.timer.timer",
    "libs.timer.timer_proxy",

    "libs.event.event_mgr",
    "libs.event.event_proxy",
    "libs.event.event_binder",

    "libs.data_struct.random_hash",

    "libs.game_net.game_net_def",
    "libs.game_net.game_net",

    "libs.csharp.csharp_help",
    "libs.csharp.csharp_type",

    "libs.state_mgr.state_mgr",
    "libs.state_mgr.state_base",
    "libs.state_mgr.composite_state_base",

    "libs.proto_parser.proto_parser",
    "libs.proto_parser.proto_store_base",
    "libs.proto_parser.protobuf_store",
    "libs.proto_parser.sproto_store",

    "common.init_global_vars"
}
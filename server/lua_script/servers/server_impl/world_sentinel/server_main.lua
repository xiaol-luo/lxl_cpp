...
function WorldSentinelServer:_on_init()
    ...
    for _, v in ipairs(self.init_setting.redis_service.element) do
        if is_table(v) and v.name == Const.consistent_hash  then
            self.redis_consistent_hash_setting = RedisServerConfig:new()
            self.redis_consistent_hash_setting:parse_from(v)
        end
    end
    if not self.redis_consistent_hash_setting or not self.redis_consistent_hash_setting.host then
        return false
    end

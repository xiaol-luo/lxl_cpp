...
std::string get_formatted_current_datetime(bool with_milliseconds)
{
	...
	long long now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(tp.time_since_epoch()).count();
    time_t current_seconds = now_ms / 1000;;
    struct tm result;
    result.tm_isdst = 0;
#ifdef _WIN32
	localtime_s(&result, &current_seconds);
#else
    ...
    if (with_milliseconds)
    {
        snprintf(datetime_buffer, sizeof(datetime_buffer)
            ...
            ,result.tm_hour, result.tm_min, result.tm_sec, (unsigned int)(now_ms % 1000));
    }
    else
    {
        snprintf(datetime_buffer, sizeof(datetime_buffer)

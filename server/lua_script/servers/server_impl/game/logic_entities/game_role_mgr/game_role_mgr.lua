...
function GameRoleMgr:_on_update()
    -- log_print("GameRoleMgr:_on_update")
end
...
function GameRoleMgr:_rpc_rsp_launch_role(role_id, session_id, rpc_error_num, launch_error_num)
    local role_state = self._role_id_to_role_state[role_id]
...
---@param rpc_rsp RpcRsp
function GameRoleMgr:_handle_remote_logout_role(rpc_rsp, user_id, role_id)
    rpc_rsp:respone(Error_None)
end
